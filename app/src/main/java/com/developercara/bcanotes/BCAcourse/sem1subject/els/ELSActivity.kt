...
import com.developercara.bcanotes.databinding.ActivityElsactivityBinding
import com.developercara.bcanotes.dataclass.Others

class ELSActivity : AppCompatActivity() {
    lateinit var binding: ActivityElsactivityBinding
    private lateinit var recyclerView: RecyclerView
    private lateinit var adapter: elsAdapter
    private val cList = mutableListOf<Others>()
    override fun onCreate(savedInstanceState: Bundle?) {
        binding = ActivityElsactivityBinding.inflate(layoutInflater)
        super.onCreate(savedInstanceState)
        setContentView(binding.root)

        recyclerView = binding.recyclerc
        adapter = elsAdapter(this, cList)
